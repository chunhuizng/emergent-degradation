...
standard_train_model_list = ["gcn_ln",
                             "gcn",
                             "robustgcn",
                             "gin",
                             "gat",
...
adv_train_model_list = ["gatguarddp_at"]
...
def build_model(model_name, num_features, num_classes):
    ...
    if model_name in ["gatdp_moe", "gatdp_moe_at"]:
        ...
        model = GATDP_moe(in_features=num_features,
                          ...
                          dropout=0.5)
        # num_experts = 4, k = 1, i + 1, base_attack = 100
        train_params = {
            "lr": 0.001,
            "n_epoch": 5000,
            "early_stop": True,
    ...
    if model_name in ["gcnguard"]:
        ...
        return model, train_params
    if model_name in ["gatguard", "gatguard_at"]:
        ...
        return model, train_params
    if model_name in ["gatguarddp", "gatguard_at"]:
        from gatguarddp import GATGuardDP

        model = GATGuardDP(in_features=num_features,
                         out_features=num_classes,
                         hidden_features=128,
                         n_heads=4,
                         n_layers=3,
